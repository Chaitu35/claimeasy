...
func (uq *UserQuery) QueryPermissions() *PermissionsQuery {
	query := (&PermissionsClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithPermissions(opts ...func(*PermissionsQuery)) *UserQuery {
	query := (&PermissionsClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPermissions; query != nil {
		if err := uq.loadPermissions(ctx, query, nodes,
			func(n *User) { n.Edges.Permissions = []*Permissions{} },
			func(n *User, e *Permissions) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadPermissions(ctx context.Context, query *PermissionsQuery, nodes []*User, init func(*User), assign func(*User, *Permissions)) error {
...
func (uq *UserQuery) loadPasswordResetTokens(ctx context.Context, query *PasswordResetTokenQuery, nodes []*User, init func(*User), assign func(*User, *PasswordResetToken)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.PasswordResetTokensTable)
		s.Join(joinT).On(s.C(passwordresettoken.FieldID), joinT.C(user.PasswordResetTokensPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.PasswordResetTokensPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.PasswordResetTokensPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*PasswordResetToken](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "password_reset_tokens" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
